...
export default function Dom() {
	...
	function addTaskToProject(title, dueDate, priority) {
		if (selectedProject) {
			const newTask = create.createTodo(
				title,
	...
	function renderTasks(task, isEdited = false) {
		...
		if (isEdited) {
			const previousTaskElement = taskContainer.querySelector(
		...
		projectList.forEach((project) => {
			project.projectToDos.forEach((task) => {
				...
				if (
					...
					isHighPriority ||
					(istaskToday && filters.dueDate === formattedDate)
				) {
					renderTasks(task);
				}
			});
	...
	function removeTask(taskElement) {
		...
		const standAloneTaskIndex = standAloneTasks.findIndex(
			(t) => t.title === taskName
		);
		...
		for (const project of projectList) {
			const projectTaskIndex = project.projectToDos.findIndex(
				(t) => t.title === taskName
			);
	...
	taskContainer.addEventListener("click", (e) => {
		...
		if (editIcon) {
			...
			if (standAloneTask) {
				...
				for (const project of projectList) {
					const projectTask = project.projectToDos.find(
						(t) => t.title === taskName
					);
					if (projectTask) {
		...
		if (squareIcon || squareIconChecked) {
			const taskElement = squareIcon
				? squareIcon.closest(".task-box")
				: squareIconChecked.closest(".task-box");
			taskElement.classList.toggle("checked-task");
			...
			const taskName = taskElement.querySelector(".task-name").textContent;
			const task =
				standAloneTasks.find((t) => t.title === taskName) ||
				projectList
					.reduce((acc, project) => acc.concat(project.projectToDos), [])
					.find((t) => t.title === taskName);
